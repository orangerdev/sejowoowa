...
class Sejowoowa_Commission {
	...
	public function set_default_option_fields() {
		...
		$default = array(
        	'wa_commission_active' 		=> $this->is_active(),
    		'wa_commission_message' 	=> 'Komisi affiliasi untuk anda dari {site_title} ({site_url}) order #{order_number} tanggal {order_date} sebesar {total_commission} rincian {detail_commission}'
	    );
	...
	public function initialize_options() {
	    ...
	    add_settings_section(
	        ...
	        'Notifikasi Komisi',           				// Title to be displayed on the administration page
	...
	public function send_message( \WC_Order $order, array $commission_data ) {
		...
		if( ! $this->is_active() ) {
			error_log( 'active '.$this->is_active() ); //debug
	    	return;
	    }
	    ...
	    $admin_phone 	= $sjw_setting->get_option_value('admin_phone');
	    
	    // Create class
		$api 			= new SJW_Woowandroidv2();
		$result 		= $api->do_post( $csid, $replaced_message, $admin_phone );
		error_log( 'result ' . print_r( $result, true ) ); //debug
	    return $result;
	}
